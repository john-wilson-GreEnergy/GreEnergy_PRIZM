...
const stringBucketStart = code.lastIndexOf('export function buildStringBucketSummary');
let stringBucketEnd = code.indexOf('function buildStatusCodeDescriptionMap', stringBucketStart);
if (stringBucketEnd === -1) stringBucketEnd = code.length;
...
code = code.substring(0, stringBucketStart) + newStringBucket + code.substring(stringBucketEnd);

const activeStart = code.lastIndexOf('// Part K - Active Issue Groups');
let activeEnd = code.indexOf('function buildStatusCodeDescriptionMap', activeStart);
if (activeEnd === -1) activeEnd = code.length;

const newActive = `// Part K - Active Issue Groups
...
code = code.substring(0, activeStart) + newActive + '\n' + code.substring(activeEnd);
