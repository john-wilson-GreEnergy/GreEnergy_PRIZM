#!/usr/bin/env bash
# BUILD: v17 (clear submenu)

set -euo pipefail

# --- script self (for reliable restarts) ---
SCRIPT_SELF="$(readlink -f "${BASH_SOURCE[0]}" 2>/dev/null || echo "${BASH_SOURCE[0]}")"
# capture original args for exec-based menu resets
ORIG_ARGS=("$@")
...
brand_text_block() {
  cat >&2 <<EOF
${BOLD}${GREEN}GreEnergy Resources LLC V1.0${RESET}
...
banner() {
  ...
  echo "Build: v17 (clear submenu)" >&2
  ...
  while true; do
    ...
    case "$choice" in
      ...
      1)
        ...
        read -r arrays </dev/tty || return 0
...
set_timeout_for_ip() { curl -sS -X POST "$(sim_base_url "$1")/timeoutminutes/$2" >/dev/null || return 1; }
...
ask_tri_open_closed_skip() {
  local prompt="$1" ans
  while true; do
    echo >&2
    echo "${prompt}" >&2
    echo "  s) Skip (leave unchanged)" >&2
    echo "  c) Closed" >&2
    echo "  o) Open" >&2
    printf "Choice [s/c/o]: " >&2
    read -r ans </dev/tty || return 1
    ans="${ans,,}"
    case "$ans" in
      s|skip|"") echo ""; return 0 ;;
      c|closed) echo "true"; return 0 ;;
      o|open)   echo "false"; return 0 ;;
      b|B) return 1 ;;
      0) exit 0 ;;
      *) echo "Invalid choice. Use s, c, or o." >&2 ;;
    esac
  done
}

payload_inner() {
  # Extract inner "values" array contents from payload JSON.
  # If payload is empty or not in expected form, prints empty.
  local p="$1"
  [[ -z "$p" ]] && { echo ""; return 0; }
  sed -E 's/^\{"values":\[(.*)\]\}$/\1/' <<<"$p"
}
...
payload_append() {
  local payload="$1" add_values="$2"
  [[ -z "$add_values" ]] && { echo "$payload"; return 0; }
  local inner
  inner="$(payload_inner "$payload")"
  if [[ -z "$inner" || "$inner" == "$payload" ]]; then
    # Fallback: treat as raw inner list
    echo "$(payload_values "$add_values")"
  else
    echo "$(payload_values "${inner},${add_values}")"
  fi
}

maybe_add_doors_to_action() {
  # Mutates COMMAND_PAYLOAD and EXPECT_LABEL (globals) by appending door values.
  local want
  want="$(ask_bool "Also set door states (Top Cap / Battery Door / AC Door)?")"
  [[ "$want" != "true" ]] && return 0
  ...
  v="$(ask_tri_open_closed_skip "Lower Top Cap")" || return 1
  if [[ -n "$v" ]]; then
    adds+=("$(value_obj "LowerTopcapClosed" "BOOLEAN" "$v" "true = door closed")")
    labels+=("LowerTopcapClosed=$v")
  fi

  v="$(ask_tri_open_closed_skip "Battery Door")" || return 1
  if [[ -n "$v" ]]; then
    adds+=("$(value_obj "BatteryDoorClosed" "BOOLEAN" "$v" "true = door closed")")
    labels+=("BatteryDoorClosed=$v")
  fi

  v="$(ask_tri_open_closed_skip "AC Door")" || return 1
  if [[ -n "$v" ]]; then
    adds+=("$(value_obj "AcDoorClosed" "BOOLEAN" "$v" "true = door closed")")
    labels+=("AcDoorClosed=$v")
  fi

  if (( ${#adds[@]} )); then
    local add_csv
    add_csv="$(IFS=,; echo "${adds[*]}")"
    COMMAND_PAYLOAD="$(payload_append "$COMMAND_PAYLOAD" "$add_csv")"
    EXPECT_LABEL+=" + Doors: $(IFS=' '; echo "${labels[*]}")"
  fi
}
select_action_menu() {
  echo >&2
  ...
  while true; do
    ...
    case "$choice" in
      ...
      1)
        ...
        maybe_add_doors_to_action || return 1
        return 0 ;;
      2)
        ACTION_KIND="commands"
...
HEADER=$'Array\tIP\tStage\tHVAC1\tFanL\tFanH\tComp\tHpump\tFreeze\tHVAC2\tFanL\tFanH\tComp\tHpump\tFreeze\tElectHeat\tSimulated\tSimTime'
  ...
  if command -v perl >/dev/null 2>&1; then
    perl -e '
      ...
      while (my $line=<STDIN>) {
        ...
        for my $i (4,5,6,7,10,11,12,13,15,16) {
...
check_ip() {
  ...
  for v in "$Stage" "$HVAC1" "$FanL1" "$FanH1" "$Comp1" "$Hpump1" "$Freeze1" \
           ...
           "$ElectHeat" "$Simulated" "$SimTime"; do
...
# ------------------------------------------------------------
# Deploy helper: run parallel deploy and compute ok/fail safely
# ------------------------------------------------------------
deploy_and_count() {
  local targets="$1"
  local minutes="$2"
  ...
  if (( xargs_rc == 0 )); then
    ...
    while IFS= read -r ip; do
      if run_deploy_on_target "$ip" "$minutes" "$kind" "$payload"; then
        ((ok++))
      else
        ((fail++))
      fi
...
clear_post_menu() {
  echo >&2
  echo "After Clear All:" >&2
  echo "  1) Validate these targets" >&2
  echo "  2) Run Clear All again" >&2
  echo "  m) Main Menu" >&2
  echo "  0) Exit" >&2
  printf "Choice: " >&2
}
...
# Run deploy (parallel) and count results
local d_total d_ok d_fail
read -r d_total d_ok d_fail < <(deploy_and_count "$targets" "$minutes" "$ACTION_KIND" "${COMMAND_PAYLOAD:-}")

echo >&2
echo "Commanded: ${d_total}  Successful: ${d_ok}  Failed: ${d_fail}" >&2

# After "Clear All", present a small submenu (like validation) so the user can
# explicitly choose what to do next.
if [[ "$ACTION_KIND" == "clear" ]]; then
  while true; do
    clear_post_menu
    local c
    read -r c </dev/tty || return 0
    case "$c" in
      m|M) return 0 ;;
      1)
        local d
        ...
        validate_targets_live "Clear All" "$targets"
        ;;
      2)
        echo >&2
        echo "Re-running Clear All..." >&2
        read -r d_total d_ok d_fail < <(deploy_and_count "$targets" "$minutes" "$ACTION_KIND" "${COMMAND_PAYLOAD:-}")
        echo >&2
        echo "Commanded: ${d_total}  Successful: ${d_ok}  Failed: ${d_fail}" >&2
        ;;
...
main() {
  ...
  while true; do
    main_menu
    read -r choice </dev/tty || continue
    case "$choice" in
      1) mode_simulate ;;
