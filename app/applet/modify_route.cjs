...
const origInnerEnd = originalCode.lastIndexOf('        res.json(responseData);', end);
...
export async function refreshSiteOperationsSources() {
    if (siteOperationsRefreshInFlight) {
...
router.get("/summary", async (req, res) => {
    ...
    try {
        ...
        // Use in-memory hot cache if available
        ...
        if (responseData) {
            cacheState = (siteOperationsRefreshInFlight || shouldRefresh) ? "STALE" : "LIVE";
        }
        ...
        // If forced refresh and we don't have data, wait up to 3s
        ...
        responseData.debug = {
             timings: {
                 totalMs,
                 cacheReadMs: tCacheRead,
                 buildMs: tBuild - tCacheRead,
                 sourceHealthMs: 0,
        ...
        if (totalMs > 500) {
            console.log(\`[SiteOps] Slow summary response: \${totalMs}ms\`);
        }
...
let result = originalCode.slice(0, start) + newCacheLogic + '\n' + originalCode.slice(end);

fs.writeFileSync('src/server/siteOperations.ts', result);
console.log('Replaced summary route.');
