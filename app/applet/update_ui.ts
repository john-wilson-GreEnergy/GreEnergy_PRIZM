...
const replaceStr = `filtered.map((s:any, i: number) => {
                  const isArrFirst = i === 0 || filtered[i-1].arrayNumber !== s.arrayNumber;
                  ...
                  <tr key={s.id} onClick={() => setSelectedString(s)} className="group hover:bg-prizm-primary/5 cursor-pointer transition-colors relative">\n                    <td className="px-3 py-1.5 border-r border-prizm-border/20 sticky left-0 group-hover:bg-prizm-surface-strong bg-prizm-surface z-20 w-[30px]" onClick={e => e.stopPropagation()}>
                        <div className="flex flex-col items-center gap-1">
                            {isArrFirst ? (
                                <input 
                                    type="checkbox" 
                                    className="w-3.5 h-3.5 accent-prizm-primary cursor-pointer" 
                                    title="Select Array"
                            ...
                            <input 
                                type="checkbox" 
                                className="w-3.5 h-3.5 accent-prizm-info cursor-pointer opacity-60 hover:opacity-100"
                                title="Select String"
