...
`;
// Replace right after getting appsCandidates
...
function getFeatherSpaceHumidity(f: any): number | null {
    ...
    return h !== undefined ? Number(h) : null;
}
...
siteOps = siteOps.replace(/fDevices.forEach\(\(f: any\) => \{[\s\S]*?if \(maxT === null \|\| t > maxT\) maxT = t;[\s\S]*\}\);/, (match) => {
    // We rewrite the inner loop for Feather Summary to use the helpers for maxT
...
// Also update where the occurrences are pushed:
