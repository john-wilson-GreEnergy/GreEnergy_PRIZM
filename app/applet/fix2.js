...
if (!file.includes('featherCellTempExcludedCollectionSegments')) {
    const searchString = \`        let maxCT: number | null = null;
        const devicesWithIssues: any[] = [];

        fDevices.forEach((f: any) => {\`;
    
    // We will just replace 'let maxCT: number | null = null;' up to '});\\n' 
    // wait that's hard. Let's just do an exact match using indexOf
...
console.log('Fixed siteOperations pt2.js correctly.');
