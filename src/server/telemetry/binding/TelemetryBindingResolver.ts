...
import { bindingFingerprint, compactTelemetryValue, deterministicBindingId, freezeBindingValue, telemetryFingerprintValue, BINDING_SCHEMA_VERSION, TELEMETRY_SNAPSHOT_VERSION } from './TelemetryBindingSnapshot';
...
const readiness = (expected: number, bound: number, stale: number, blockers: string[]): TelemetryBindingReadiness => freezeBindingValue({ ready: blockers.length === 0 && expected === bound, blockers: [...new Set(blockers)], expected, bound, stale });
...
export class TelemetryBindingResolver {
  async build(input: TelemetryBindingBuildInput): Promise<TelemetryBindingSnapshot> {
    ...
    const bindingsByDomain = freezeBindingValue({ 'string-telemetry': stringBindings, 'controller-health': controllerBindings });
  ...
  private index(bindings: readonly TelemetryObservationBinding[]): TelemetryBindingIndexes {
    ...
    return freezeBindingValue({ byBindingId, byObjectId, byCanonicalKey, byDomain, stringByCoordinate, stringsByEnergySegment, controllerHealthByObjectId });
  }
}
