...
export default function ThermalSiteOverview({units,metric,selected,onChoose,onRank}:Props){
  ...
  return <section aria-label="Whole-site thermal overview" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-bold">Whole-site outlier overview · {thermalMetrics[metric].label}</h2><button className="rounded border px-3 py-2 text-xs font-semibold" onClick={onRank}>Open target list</button></div>
    <p className="mt-2 text-xs text-slate-600">Latest cached readings across all arrays, independent of the filters above. {points.length} of {units.length} HVAC units plotted. Click a point to inspect its current readings. Each position is a segment, not a time sample.</p>
