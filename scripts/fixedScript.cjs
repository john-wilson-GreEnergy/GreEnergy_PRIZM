...
const appSearchReplacement = `
        if (!appsCandidates.length) appsCandidates = findArraysByObjectKeys(status, ["appCode"]);
...
siteOps = siteOps.replace(
    "if (f.lostComms || f.devicesWithLostComms?.length > 0 || f.lostCommsDevices?.length > 0 || f.deviceStatusComms?.includes('Lost'))",
    "if (hasLostComms(f))"
);
siteOps = siteOps.replace(
...
siteOps = siteOps.replace(
    "if (!f.reachable || f.lostComms || f.fssValid === false || f.thermalData?.fssSignals?.valid === false || f.doorsValid === false || f.doors?.valid === false || (f.warningCount > 0) || (f.alarmCount > 0)) {",
...
const searchEnclosure = "const enclosureLabel = `Array ${st.arrayNumber || st.array || st.arrayIndex || st.Array || '?'} ES${st.stringNumber || st.string || st.stringIndex || st.String || '?'}`;";

siteOps = siteOps.replace(searchEnclosure, genericWarnStr);
...
siteOps = siteOps.replace('bessFleetSummary: {},', 'bessFleetSummary: bessFleetSummary,');

fs.writeFileSync('src/server/siteOperations.ts', siteOps);
console.log('Fixed exactly using JS');
