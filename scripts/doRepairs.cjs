...
siteOps = siteOps.replace(/let maxCT = 0;/g, 'let maxCT = -999;');
siteOps = siteOps.replace(/let maxST = 0,/g, 'let maxST = -999,');
siteOps = siteOps.replace(/const st = f\.spaceTemperature \|\| \(f\.rawResponse\?\\.thermalData\?\\.spaceTemperature\) \|\| \(f\.rawResponse\?\\.thermalData\?\\.spaceTemp\) \|\| 0;/g, 'const st = getFeatherSpaceTemp(f) ?? -999;');
siteOps = siteOps.replace(/if \(f\.avgCellTemperature && f\.avgCellTemperature > maxCT\) maxCT = f\.avgCellTemperature;/g, 'const ct = getFeatherCellTemp(f) ?? -999;\\n            if (ct && ct > maxCT) maxCT = ct;');
siteOps = siteOps.replace(/!f\.reachable \|\| f\.lostComms/g, '!f.reachable || hasLostComms(f)');
...
const bestFleetPopulateStr = `
        const bessFleetSummary: any = {
            ...
            onlineStrings: stringSummary.buckets?.online || 0,
            nearlineStrings: stringSummary.buckets?.nearline || 0,
            offlineStrings: stringSummary.buckets?.offline || 0,
            notCommunicatingStrings: stringSummary.buckets?.notCommunicating || 0,
